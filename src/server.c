...
int loop()
{
    ...
    if (client_id < 0) {
        if (errno == ENOMEM) {
            printTop("\n[DEBUG] ACCEPT: Out of memory (Geister-Sockets?).\n");
            printTop("[DEBUG] Warte auf Garbage Collection...\n");
            svcSleepThread(1000000000LL); // 1 Sekunde warten, 3DS OS aufräumen lassen
