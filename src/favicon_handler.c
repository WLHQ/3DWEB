...
http_response *get_favicon_icon(http_request *request)
{
    ...
    // Check user agent to decide what to serve
    int is_modern = check_browser_compatibility(request->agent);

    // Currently we default to SVG for everyone as it's the only asset we have hardcoded.
    // However, this structure allows us to easily add a PNG fallback or similar logic.
    // If we had a PNG, we would do:
    // if (is_modern) { serve SVG } else { serve PNG }

    // For now, regardless of the check result, we serve the SVG.
    (void)is_modern; // Silence unused variable warning if compiler is strict

    response->code = 200;
    response->content_type = strdup("Content-Type: image/svg+xml\n");

    size_t svg_len = strlen(FAVICON_SVG);
    char *payload = memalloc((svg_len + 1) * sizeof(char));
    if (payload) {
        strcpy(payload, FAVICON_SVG);
    }

    response->payload = payload;
    response->payload_len = svg_len;
